...
class InterTableMend(Ui_MainWindow, QMainWindow):
    def __init__(self):
        ...
        img = QImage('mendeleev.jpg')
        img = img.scaled(141, 131, Qt.KeepAspectRatio)
        pixmap = QPixmap(img)
    ...
    def keyPressEvent(self, event):
        ...
        if int(event.modifiers()) == (Qt.ControlModifier + Qt.ShiftModifier):
            if event.key() == Qt.Key_B:
                ...
                msg.setText('Вы перешли в режим поиска.')
    ...
    def modify(self):
        ...
        all_, ok = QInputDialog.getMultiLineText(self, 'Свойства нового элемента',
                                                 ...
                                                 '\n\nПишите без единиц измерения, все дробные числа через точку.')
        if not ok:
            self.modify()
    ...
    def do_search(self):
        ...
        if search_ask == '':
            ...
            if parameter == params[0]:
                ...
                query.exec(f'''SELECT * FROM elements
                WHERE name = "{search_ask.lower().capitalize()}"''')
        ...
        if not all(lst_val):
            # Предупреждение о неправильности ввода
            msg = QMessageBox()
            msg.setWindowTitle('Ошибка!')
            msg.setText('Запрос составлен неправильно или такого элемента не существует.')
            msg.setIcon(QMessageBox.Warning)
            msg.exec_()
